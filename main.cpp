...
// Add a boolean to all elements that are connected to the main, then the changes check if the boolean is set-
// If not then dont start 'Update_Renderer()'
...
void Old_Code(int Argument_Count, char** Arguments) {
    srand(time(NULL));
...
void Campaing(GGUI::Window* Main){
    ...
    Map_Canvas = new GGUI::Canvas(
        Screen_Division_Width -1,
        Screen_Division_Height,
        GGUI::Coordinates(Screen_Division_Width + 1, 0)
    );
